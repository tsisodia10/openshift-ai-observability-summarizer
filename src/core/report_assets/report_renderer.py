...
def generate_pdf_report(report_schema: ReportSchema) -> bytes:
    ...
    Raises:
        RuntimeError: If WeasyPrint is not available for PDF generation
    """
    
    if not WEASYPRINT_AVAILABLE:
        raise RuntimeError(
            "PDF generation is not available. WeasyPrint library is not installed or missing dependencies. "
            "Please install WeasyPrint or use HTML/Markdown format instead."
        )
    ...
    try:
        ...
        # Re-raise the exception instead of returning confusing HTML content
