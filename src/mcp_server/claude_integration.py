...
try:
    ...
    try:
        ...
        pylogger_path = os.path.join(os.path.dirname(__file__), 'utils', 'pylogger.py')
        spec = importlib.util.spec_from_file_location("pylogger", pylogger_path)
        pylogger = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(pylogger)
        get_python_logger = pylogger.get_python_logger
...
class PrometheusChatBot:
    ...
    def chat(self, user_question: str, namespace: Optional[str] = None, scope: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            return "Conversation exceeded maximum iterations. Please try a simpler question."
